...
function handleItemSubmit() {
    const selectedSku = document.getElementById('itemSkuSelect').value;
    // Use the currently selected product's data instead of the global appState
    ...
    // Get stock from the detailed data we fetched
    const stock = skuInfo.stock ? parseInt(skuInfo.stock) : 0;
    
    const newItem = { 
        sku: selectedSku, 
        description: currentItemData.description, // Use the correct description
    ...
    if (editingItemIndex !== null) {
        ...
        if (existingItemIndex > -1) {
            ...
            appState.orderItems[existingItemIndex].quantity += newItem.quantity;
            appState.orderItems[existingItemIndex].price += newItem.price;
        } else {
    ...
    updateOrderSummary();
    stopEditingItem(); // This will also call clearItemInputs
}
