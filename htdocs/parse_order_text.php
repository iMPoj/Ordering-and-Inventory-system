...
require __DIR__ . '/db_connect.php'; // Connect to the database
        ...
        if (preg_match('/(\d{8})\s+([0-9,.]+)\s+(PC|PK|BX|SET|BOXOF\d+|PKOF\d+)\s+([0-9,]+)\s+0\s+0/i', $line, $data_matches)) {
            ...
            for ($j = 1; $j <= 3; $j++) {
                if (isset($lines[$i - $j])) {
                    ...
                    if (strpos($prev_line, 'VENDOR ITEM CODE') !== false || strpos($prev_line, 'BARCODE') !== false) break; 
                }
            }
...
if ($customerName) {
    $stmt = $pdo->prepare("SELECT id FROM customers WHERE name LIKE ? LIMIT 1");
...
$data = [
    ...
    'customerId' => $customerId, // This is the new, important field
    'shipTo' => find_value('/Delivery\/Ship To\s*([^\n\r]+)/i', $text),
    'items' => $items
];

if (!$data['poNumber'] || !$data['customerName'] || empty($data['items'])) {
    echo json_encode(['success' => false, 'message' => 'Could not parse required fields. Check text. Found ' . count($items) . ' items.']);
    exit;
}

echo json_encode(['success' => true, 'message' => 'PDF text parsed successfully.', 'data' => $data]);
?>