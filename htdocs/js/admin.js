...
async function exportData(format) {
    ...
    result.data.forEach(order => {
        order.items.forEach(item => {
            const so_number = Array.isArray(order.so_number) ? order.so_number.join(';') : (order.so_number || '');
            const row = [order.date.split(' ')[0], order.location, order.bu, order.customer.name, order.customer.address, order.customer.poNumber, order.customer_code || '', so_number, item.sku, item.description, item.quantity, item.price, item.status];
...
export function initAdmin(dataCallback) {
    ...
    document.getElementById('parsePdfTextBtn')?.addEventListener('click', async () => {
        ...
        if (!rawText.trim()) return showMessage("Please paste text from the PDF first.", true);
        showLoader();
        ...
        try {
            ...
            if (result.success) {
                sessionStorage.setItem('pdfOrderData', JSON.stringify(result.data));
                window.location.href = 'pdf_to_order.php';
            } else { showMessage(result.message || "Failed to parse text.", true); }
        } catch (error) {
    ...
    // Function to set the dropdowns to the current date.
    ...
    // Load the current setting when the admin page initializes.
