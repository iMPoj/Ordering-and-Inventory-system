...
function bulkUpdateStock($pdo) {
    ...
    try {
        ...
        // --- THIS IS THE FIX ---
        // This line is now UNCOMMENTED. It will reset all stock for the location to 0 first.
        ...
        foreach ($lines as $line) {
            ...
            // Simpler parsing: Assumes SKU is first, Quantity is second-to-last
            $parts = preg_split('/\s+/', trim($line), -1, PREG_SPLIT_NO_EMPTY);
            if (count($parts) < 2) continue; // Need at least SKU and Quantity

            $code = $parts[0];
            // Get the second to last part, assuming it's the quantity
            $quantityStr = str_replace(',', '', $parts[count($parts) - 2]);

            // Validate SKU and Quantity
            if (!is_numeric($code) || !is_numeric($quantityStr)) {
                // Skip lines with invalid numeric values for SKU or Quantity
                continue;
            }
        ...
        // Let the main error handler format the error
        throw $e;
    }
    exit;
}
...
function getSalesSummaryData($pdo) {
    ...
    $baseSql = "SELECT
                    oi.status, oi.price, oi.quantity, p.bu,
                    pc.sales_price
                FROM orders o
                JOIN order_items oi ON o.id = oi.order_id
                JOIN customers c ON o.customer_id = c.id
    ...
    foreach ($allItems as $item) {
        if ($item['bu']) {
            ...
            if ($item['status'] === 'served') {
                $buPerformance[$item['bu']]['served_gross'] += $gross_value;
                $buPerformance[$item['bu']]['served_net_vat_ex'] += (float)$item['price'];
            } else {
    ...
    foreach ($buPerformance as &$buData) {
        $buData['po_amount_total'] = ($buData['served_gross'] ?? 0) + ($buData['unserved_gross'] ?? 0);
        $net_vat_ex = $buData['served_net_vat_ex'];
        $buData['vat_amount'] = $net_vat_ex * ($VAT_RATE - 1);
        $buData['served_net_vat_in'] = $net_vat_ex * $VAT_RATE;
    }
    unset($buData);
